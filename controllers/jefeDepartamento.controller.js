...
export const loginJefeDepartamento = async (req, res) => {
    try {
        ...
        const docenteFind = await docente.findOne({where: { numeroEmpleadoDocente: id}}) //aqui estuvo Ericka
        ...
        return res.status(200).json({ message: "Login exitoso", token, expiresIn,jefe:jefeDepartamentoLogin, docente: docenteFind }); //aqui estuvo Ericka
    } catch (error) {
        console.log(error);
...
export const notasIngresadasDocentesDepartamento = async (req, res) => {
   ...
   const seccionFound = await seccion.findOne({where:{idSeccion: idseccion}})
   ...
   return res.status(200).json({message: "calificaciones" , calificaciones: registrosCalificacionesArray})
};
...
export const obtenerEvaluacionesDocentesDepartamento = async (req, res) => {
    ...
    const matriculaFound = await matricula.findAll({where: {idSeccion: idSeccion}});
    if(!matriculaFound){
    ...
    for( const matricula of matriculaFound){
        ...
        const evaluacionesFound = await evaluacion.findOne({where: {idMatricula: idMatricula}});
    ...
    if(evaluacionesFoundArray == []){
        ...
        return res.status(200).json({ message: "Evaluaciones de docentes", evaluaciones: evaluacionesFoundArray});
    }
};  

export const obtenerEvaluacion = async(req, res) => {
    const {idEvalucion} = req. body;
    const evaluacionFound = await evaluacion.findOne({where: { idEvalucion: idEvalucion}})
    if(!evaluacionFound){
        return res.status(400).json({ message: "No hay evaluaciones" });
    }
    return res.status(200).json({ message: "Evaluaciones de docentes", evaluacionFound });
}
...
export const restablecerContraseña = async (req, res) => {
    const { numeroEmpleadoDocente } = req.body ;
    const docentesFounds = await docente.findOne({where: {numeroEmpleadoDocente : numeroEmpleadoDocente}});
    if(!docentesFounds){
        return res.status(400).json({ message: "No hay docentes" });
    };
    const { correo , nombres, apellidos } = docentesFounds.dataValues;
    let docenteInfo = { correo , nombres, apellidos };
    const token = jwt.sign({ numeroEmpleadoDocente }, process.env.JWT_SECRET, {expiresIn : "2m"});
...
export async function enviarCorreo(docente, resetLink) {
    ...
    try {
        const mailOptions = {
        from: "Administradores" + "<" + process.env.EMAIL_USER + "@gmail.com" + ">",
        to: `${docente.correo}`,
        subject: "Restablecer Clave",
        Text: `Para restablecer tu clave, haz clic en el siguiente enlace: ${resetLink}`,
        html: `
                <h1>Un gusto saludarte, <strong>${docente.nombres + " " + docente.apellidos}<strong>.</h1>
...
export const obtenerDocente = async (req,res) => {
    ...
    const docenteFound = await docente.findAll({where:{centroRegional,nombreCarrera}})
    if(!docenteFound){
        return res.status(400).json({ message: "No hay docentes" });
    };
    let arraySecciones = []
    ...
    return res.status(200).json({ message: "Docente encontrado", secciones: arraySecciones});
}
...
export const listadoEstudiantesMatriculado = async (req,res) => {
    ...
    const jefeDepartamentoFound = await docente.findOne({where:{numeroEmpleadoDocente}})
    const {nombreCarrera} = jefeDepartamentoFound.dataValues
    const matriculasFound = await matricula.findAll({where:{nombreCarrera}})
    let arrayMatriculas = []
...
export const obtenerAsignaturasDepartamento = async (req,res) => {
    ...
    const asignaturasFound = await asignatura.findAll({where: {nombreCarrera : nombreCarrera, centroRegional: centroRegional}});
    if(!asignaturasFound){
        return res.status(400).json({ message: "No hay asignaturas" });
    }
    return res.status(200).json({ message: "Listado de asignaturas", asignaturasFound });
...
export const obtenerDocentesDepartamento = async (req,res) => {
    ...
    const docenteFound = await docente.findAll({where:{nombreCarrera, centroRegional}})
    if(docenteFound.length == 0){
        return res.status(400).json({ message: "No hay docentes" });
    };
    return res.status(200).json({ message: "Docente encontrado", docenteFound});
};
...
export const obtenerEdificios = async (req,res) => {
    ...
    const edificiosFound = await edificio.findAll({where:{centroRegional}})
    if(edificiosFound.length == 0){
        return res.status(400).json({ message: "No hay edificios" });
    };
    return res.status(200).json({ message: "Edificios encontrados", edificiosFound});
};

export const obtenerAulas = async (req,res) => {
    const { nombreEdificio } = req.body
    const edificiosFound = await edificio.findOne({where:{nombreEdificio}})
...
export const crearSeccion = async (req,res) => {
    try {
        const {edificio, aula, horaInicial, horaFinal, cupos, dias, nombreAsignatura} = req.body
        let {nombreDocente} = req.body
        let partes = nombreDocente.split(" ");
        let nombres = partes[0] + " " + partes[1];
        ...
        const asignaturaFound = await asignatura.findOne({where:{nombreClase: nombreAsignatura, centroRegional}})
...
export const obtenerSecciones = async (req,res) => {
    ...
    return res.status(200).json({ message: "Secciones encontradas", arraySecciones});
};
...
export const cancelacionSeccion = async (req,res) => {
    ...
    const justificacion = await JustificacionCancelacionSeccion.create({nombreSeccion:  nombreSeccion, justificacion: motivo})
    const seccionDelete = await seccion.destroy({where:{idSeccion}})
    return res.status(200).json({ message: "Seccion cancelada", seccionDelete, justificacion});
};

export const actualizarCuposSeccion = async (req,res) => {
    const {idSeccion, cupos} = req.body
    const seccionUpdate = await seccion.update({cupos: cupos},{where:{idSeccion}})
    return res.status(200).json({ message: "Seccion actualizada", seccionUpdate});
};
...
export const obtenerListaEspera = async (req,res) => {
    ...
    return res.status(200).json({ message: "Lista de espera", listaEsperaFound});
};

export const obtenerListaEsperaInfo = async (req,res) => {
    const {idSeccion} = req.body
    const listaEsperaFound = await listaEspera.findAll({where:{idSeccion}})
    ...
    return res.status(200).json({ message: "Lista de espera", listaEsperaInfo});
};
...
export const aceptarEstudianteListaEspera = async (req,res) => {
    ...
    const listaEsperaDelete = await listaEspera.destroy({where:{numeroCuenta, idSeccion}})
    const estudianteFound = await estudiante.findOne({where:{numeroCuenta}})
    ...
    return res.status(200).json({ message: "Estudiante aceptado", matriculaCreate});
};